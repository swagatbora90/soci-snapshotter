...
// to the converted image. The caller is responsible for ensuring the OCI Index
...
func (b *IndexBuilder) Convert(ctx context.Context, img images.Image, opts ...ConvertOption) (*ocispec.Descriptor, error) {
	...
	indexes, err := b.buildPlatforms(ctx, img, convertCfg.platforms)
	if err != nil {
		return nil, err
	}
...
// buildPlatforms builds a SOCI index for each specified platform
func (b *IndexBuilder) buildPlatforms(ctx context.Context, img images.Image, platforms []ocispec.Platform) ([]*IndexWithMetadata, error) {
...
func (b *IndexBuilder) newOciIndex(ctx context.Context, img images.Image) (ocispec.Index, error) {
	if images.IsIndexType(img.Target.MediaType) {
		...
		// To increase comptatibility, change the media type to OCI image manifest
...
func (b *IndexBuilder) annotateImages(ctx context.Context, ociIndex *ocispec.Index, sociIndexes []*IndexWithMetadata) error {
	for i := 0; i < len(ociIndex.Manifests); i++ {
		...
		if idx >= 0 {
			...
			indexWithMetadata.Desc.Annotations[IndexAnnotationImageDigest] = manifestDesc.Digest.String()
		}
	}
	return nil
}
